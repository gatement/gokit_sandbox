...
void Motor_Control(u8 clockwiseSpeed, u8 counterClockwiseSpeed)
{
    ...
    if(clockwiseSpeed > MOTOR_MAX) 
        clockwiseSpeed = MOTOR_MAX;

    if(counterClockwiseSpeed > MOTOR_MAX) 
        counterClockwiseSpeed = MOTOR_MAX;

    TIM_SetCompare3(TIM4, clockwiseSpeed*temp);	        //m1
    TIM_SetCompare4(TIM4, counterClockwiseSpeed*temp);	//m4
}
